...
3. **Tab navigation** with underline-style main tabs and segmented-control sub-tabs:
   ...
   - **Withdrawals** — Interactive retirement withdrawal planner featuring:
     ...
     - **Summary Cards** — projected portfolio at retirement, year-1 total income (portfolio + combined SS), effective tax rate, combined Social Security card (per-participant breakdown), lifetime taxes (FV + PV), portfolio longevity, balance at longevity age (FV + PV, shown when portfolio survives)
     ...
     - **Social Security Age Recommendation** — per-participant comparison tables showing benefits at ages 62/67/70 with monthly/annual amounts, % of FRA, years collecting, inflation-adjusted lifetime totals, and optimal claiming age recommendation based on each person's life expectancy
      ...
      - **Financial Engine**:
        ...
        - Cumulative tax tracking includes depleted-year taxes (SS + state + IRMAA continue after portfolio depletion)
        - **One-Time Spendings**: Age-targeted lump-sum expenses (e.g., home renovation, car purchase) with per-item label/amount/age. Applied on top of regular spending phases, inflation-adjusted. Persisted in localStorage.
        ...
        - **Interactive Metric Charts**: SVG charts with tab switching and PV/FV toggle (present value vs projected). Series: Account Balances (stacked), Total Balance (line), Annual Taxes (bar), After-Tax Income (line), RMD Amount (bar), Social Security (line), Withdrawals by Account (stacked), Roth Conversions (bar, conditional). Flow-type charts (taxes, income, RMD, SS, conversions) show a "Lifetime Total" annotation via opt-in `cumTotal` flag.
   ...
   - **Scenarios** — What-if scenario comparator: 3-scenario grid with per-scenario retirement age, life expectancy, SS claiming age, monthly SS income, annual return, inflation, Roth conversion strategy (None/Conservative/Moderate/Aggressive/Custom with bracket %, start/end age), state tax %, and multi-phase spending plans (age-based monthly spend phases with add/remove). Side-by-side comparison table (MC success, BETR, lifetime taxes/income, ending balance with PV) in a **collapsible panel**. Per-scenario metric charts (Account Balances default) with global chart controls (Apply to All / Individual toggle for FV/PV and chart type) in a **collapsible panel**. Global PV/FV toggle updates all values across tables and charts. Tab state persistence (comparison results, charts cached across tab switches with scenario-specific fingerprint). PDF export, localStorage persistence. Includes an **Inherit Withdrawals Glide Path** option (`scSharedGlide`) that reuses the Withdrawals tab glide schedule and disables fixed return/volatility inputs when active.
